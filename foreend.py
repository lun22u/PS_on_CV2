...
def landmark_dec_dlib_fun_for(rects, img_gray):
    land_marks = []
...
def landmark_dec_dlib_fun(img_src):
    ...
    return landmark_dec_dlib_fun_for(rects, img_gray)
...
def Face_for(_img, landmarks, face):
    thin_image = _img
    for landmarks_node in landmarks:
        ...
        # 瘦左边脸
        thin_image = localTranslationWarp(thin_image, left_landmark[0, 0], left_landmark[0, 1], endPt[0, 0], endPt[0, 1],
                                          r_left)
        # 瘦右边脸
...
def Face(_img, face):
    ...
    return Face_for(_img, landmarks, face)
...
white_ = tk.IntVar()
scale1 = Scale(top, variable=white_, bg='lightcyan', length=520, from_=0, to=100, orient='horizontal', tickinterval=50,
               resolution=1)
...
skin_ = tk.IntVar()
scale2 = Scale(top, variable=skin_, bg='lightcyan', length=520, from_=0, to=100, orient='horizontal', tickinterval=50,
               resolution=1)
...
face_ = tk.IntVar()
scale3 = Scale(top, variable=face_, bg='lightcyan', length=520, from_=0, to=100, orient='horizontal', tickinterval=50,
               resolution=1)
